...
public class Controller : IMessageHandler
{
				...
				for (int num115 = 0; num115 < GameScr.vItemMap.size(); num115++)
				{
					...
					try
					{
						...
						else if (itemMap2.template.type == 10)
						{
							...
							Char.myCharz().luongStr = NinjaUtil.getMoneys(Char.myCharz().luong);
						}
						else if (itemMap2.template.type == 34)
						{
							num = msg.reader().readShort();
							Char.myCharz().luongKhoa += num;
							Char.myCharz().luongKhoaStr = NinjaUtil.getMoneys(Char.myCharz().luongKhoa);
						}
					}
					catch (Exception)
					{
	...
	public void loadInfoMap(Message msg)
	{
		try
		{
			...
			Mod.Mod.isVaoKhu = false;
			Mod.Mod.canUpdate = true;
		}
		catch (Exception ex)
		{
