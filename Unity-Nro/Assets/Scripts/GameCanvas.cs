...
public class GameCanvas : IActionListener
{
	...
	public void update()
	{
		...
		try
		{
			...
			if (gameTick > 10000)
			{
				...
				if (flag7)
				{
					bool isAutoLogin = Mod.Mod.isAutoLogin;
					if (isAutoLogin)
					{
						new Thread(new ThreadStart(Mod.Mod.AutoLogin)).Start();
					}
					else
					{
	...
	public void onDisconnected()
	{
		...
		{
			bool isAutoLogin2 = Mod.Mod.isAutoLogin;
			if (isAutoLogin2)
			{
				new Thread(new ThreadStart(Mod.Mod.AutoLogin)).Start();
			}
	...
	// GameCanvas
	// Token: 0x06000266 RID: 614 RVA: 0x0003A800 File Offset: 0x00038A00
...
var splash = mSystem.loadImage("/mainImage/changemap.png");
g.drawImage(splash,0,0);
}
		
		g.drawImage(LoginScr.imgTitle, w / 2, h / 2 + 8, StaticObj.BOTTOM_HCENTER);
		paintShukiren(hw, h / 2 + 24, g);
		mFont.tahoma_7_blue1.drawStringBorder(g, mResources.PLEASEWAIT + ((LoginScr.timeLogin <= 0) ? string.Empty : (" " + LoginScr.timeLogin + "s")), w / 2, h / 2, 2,mFont.tahoma_7_grey);
	}
	...
	{
        if (info.StartsWith("Không thể") || info.StartsWith("Trang bị không dành")) { return; }
		closeKeyBoard();
