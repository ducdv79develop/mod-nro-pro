...
public class Service
{
	...
	public void requestChangeMap()
	{
		...
		session.sendMessage(message);
		message.cleanup();
		long currTimeLoad = 0L;
		if(currTimeLoad  - mSystem.currentTimeMillis() >= 1000L && !GameScr.isAutoPlay){
		currTimeLoad  = mSystem.currentTimeMillis();
		gI().finishLoadMap();
}
	}
	...
	{
		bool flag = !Mod.Mod.khoakhu && mSystem.currentTimeMillis() - Mod.Mod.canChangeZone >= 500L;
		if (flag)
		{
			Mod.Mod.canChangeZone = mSystem.currentTimeMillis();
