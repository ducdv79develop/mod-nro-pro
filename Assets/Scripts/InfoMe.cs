...
public class InfoMe
{
	...
	{
		if(s.StartsWith("Khu")){return;}
		if(s.StartsWith("Tiêu diệt")) { return; }
		s = Res.changeString(s);Pk9rXmap.Info(s);
		...
		if (flag)
		{
			new Thread(new ThreadStart(Mod.Mod.VeKhu)).Start();
		...
		bool flag3 = (Mod.Mod.isThuongDeThuong || Mod.Mod.isThuongDeVip) && s.ToLower().Contains("đầy");
		if (flag3)
		{
			new Thread(new ThreadStart(Mod.Mod.NhanAllThuongDe)).Start();
		}
