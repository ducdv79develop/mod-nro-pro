...
public class LoginScr : mScreen, IActionListener
{
	...
	public override void switchToMe()
	{
		...
		bool abc = true;
        if (abc) {
			if (GameCanvas.isPointerHoldIn(GameCanvas.w / 2 - 120, GameCanvas.h - 200, 60, 35) && GameCanvas.isPointerClick && GameCanvas.isPointerJustRelease)
			{
	...
	public void perform(int idAction, object p)
	{
		...
		case 1005:
			try
			{
				GameMidlet.instance.platformRequest("http://nros.fun/");
				break;
			}
			catch (Exception)
			{
