...
public class GameScr : mScreen, IChatable
{
	...
	public static void loadBg()
	{
		...
		MobCapcha.init();
		if(mGraphics.zoomLevel != 2) { isAnalog = 1; } else {isAnalog = ((Rms.loadRMSInt("analog") == 1) ? 1 : 0); }
	...
	public void resetButton()
	{
		...
		{
			if (global::Char.myCharz().meDead && Mod.Mod.IsGoBack && Mod.Mod.IdmapGB != -1)
			{
				GameScr.isAutoPlay = false;
				Service.gI().returnTownFromDead();
				new Thread(new ThreadStart(Mod.Mod.GoBack)).Start();
			}
			else
			{
	...
	private void updateKeyTouchControl()
	{
		...
		{
			int num = 95;
			for (int i = 0; i < Mod.Mod.chars.Length; i++)
			{
				if (Mod.Mod.chars[i] != null)
				{
					if (GameCanvas.isPointerHoldIn(GameCanvas.w - 150, num, 150, 10))
					{
						Mod.Mod.GotoXY(Mod.Mod.chars[i].cx, Mod.Mod.chars[i].cy);
						global::Char.myCharz().focusManualTo(Mod.Mod.chars[i]);
		...
		{
			int num = 90;
			if (GameCanvas.isPointerHoldIn(440, num + 60, 18, 12))
			{
				Xmap.LoadMapLeft();
			}
			else if (GameCanvas.isPointerHoldIn(460, num + 60, 18, 12))
			{
				Xmap.LoadMapCenter();
			}
			else if (GameCanvas.isPointerHoldIn(485, num + 60, 18, 12))
			{
				Xmap.LoadMapRight();
			}
		}
		...
		else
		{
			...
			else if (Char.myCharz().statusMe != 14)
			{
				if (gamePad.isSmallGamePad)
				{
					...
					{
						g.setColor(9670800);
						g.fillRect(xHP + 9, yHP + 10 + 8, 22, 20);
						g.setColor(16777215);
						g.fillRect(xHP + 9, yHP + 10 + 8 + ((num3 != 0) ? (20 - num3) : 0), 22, (num3 == 0) ? 20 : num3);
						g.drawImage((mScreen.keyTouch != 10) ? imgHP1 : imgHP2, xHP, yHP + 8, 0);
						mFont.tahoma_7_red.drawString(g, string.Empty + hpPotion, xHP + 20, yHP + 15 + 8, 2);
					}
					else if (isAnalog == 1)
					{
						g.drawImage((mScreen.keyTouch != 10) ? imgSkill : imgSkill2, xSkill + xHP - 1, yHP - 1, 0);
				...
				{
					g.setColor(9670800);
					g.fillRect(xHP + 9, yHP + 10 - 6 + 8, 22, 20);
					g.setColor(16777215);
					g.fillRect(xHP + 9, yHP + 10 + 8 + ((num3 != 0) ? (20 - num3) : 0) - 6, 22, (num3 == 0) ? 20 : num3);
					g.drawImage((mScreen.keyTouch != 10) ? imgHP1 : imgHP2, xHP, yHP - 6 + 8, 0);
					mFont.tahoma_7_red.drawString(g, string.Empty + hpPotion, xHP + 20, yHP + 15 + 8 - 6, 2);
				}
				else
				{
					g.setColor(9670800);
					g.fillRect(xHP + 10, yHP + 10 - 6 + 8, 20, 18);
					g.setColor(16777215);
					g.fillRect(xHP + 10, yHP + 10 + 8 + ((num3 != 0) ? (20 - num3) : 0) - 6, 20, (num3 == 0) ? 18 : num3);
					g.drawImage((mScreen.keyTouch != 10) ? imgHP3 : imgHP4, xHP + 20, yHP + 20 - 6 + 8, mGraphics.HCENTER | mGraphics.VCENTER);
					mFont.tahoma_7_red.drawString(g, string.Empty + hpPotion, xHP + 20, yHP + 15 + 8 - 6, 2);
				}
			}
			if (isHaveSelectSkill)
			{
				...
				for (int i = 0; i < num4; i++)
				{
					...
					double num999 = ((double)skill.coolDown - mSystem.currentTimeMillis() + skill.lastTimeUseThisSkill);
					mFont.tahoma_7_yellow.drawStringBorder(g, (num999 > 0) ? string.Concat(num999 / 1000) : string.Empty, GameScr.xSkill + GameScr.xS[i] + 14, GameScr.yS[i] + 8, mFont.CENTER, mFont.tahoma_7_grey);
				}
			}
			paintGamePad(g);
		}
	...
	public void SetSkill(Skill skillchoose, int skillset)
	{
		for (int i = 0; i < keySkill.Length; i++)
		{
			if (keySkill[i] != null && keySkill[i].template.id == skillchoose.template.id)
			{
				keySkill[i] = null;
			}
		}
	...
	public void chatVip(string chatVip)
	{
		...
		{
			Mod.Mod.bossVip.addElement(new ShowBoss(chatVip));
			if (Mod.Mod.bossVip.size() > 5)
			{
				Mod.Mod.bossVip.removeElementAt(0);
			}
		}
	...
	public void paintChatVip(mGraphics g)
	{
		if (vChatVip.size() != 0 && isPaintChatVip)
		{
			...
			mFont.tahoma_7_blue1.drawStringBorder(g, st, xChatVip, GameCanvas.h - 13, 0, mFont.tahoma_7);
		}
	}
